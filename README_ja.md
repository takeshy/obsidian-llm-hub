...
ボード設定は `Dashboards/Kanbans/` 配下の再利用可能な `.kanban` ファイルとして保存できます。ウィジェット設定の **現在の設定から .kanban ファイルを作成** を使うと、同じボードを複数のダッシュボードから参照できます。ヘッダーの一時タグ絞り込みに加え、表示項目は検出された frontmatter プロパティから選択でき、別名や項目名の非表示を指定できます。`file.path` / `file.name` / `file.content` / `file.mtime` / `file.ctime` は明示的に選択した場合だけ表示され、最大文字数は `file.content` に指定できます。
...
Secret Manager ウィジェットは、プラグインの既存の暗号化鍵を使い、各値を Vault 内の `.encrypted` ファイルとして保存します。名前・説明・明示的に公開したメタデータは検索できますが、秘密値はコピー時にだけ復号され、平文では保存されません。先にプラグイン設定で暗号化を設定してください。
