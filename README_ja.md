...
| **Vault: 読み取り専用** | ファイルを変更せず検索・読み取り | `read_note`、`read_timeline`、`search_notes`、`list_notes`、`list_folders`、`get_active_note_info` |
...
- **Vault: 読み取り専用** - ノートの検索・読み取りだけを許可し、作成・編集・削除・名前変更を禁止します。外部MCPやスキルツールは、それぞれの権限設定に従います。
...
**コマンド**欄には引数を含むコマンド行を貼り付けられます。その引数は**引数**欄の内容より前に追加されます。空白を含むパスや引数は `"C:\Program Files\nodejs\node.exe" "C:\my app\server.js"` のように引用符で囲みます。Windowsパスのバックスラッシュは保持されます。**接続テスト**中は編集・保存を無効にし、失敗時は起動・接続エラーを表示したまま画面を開いておきます。タイムアウト時は直近の標準エラー出力も表示し、接続に失敗した子プロセスは終了します。
...
**ツール実行の承認：** 初期状態では、実行前にサーバー名・ツール名・実行引数を表示し、「今回だけ許可」「このツールを常に許可」「拒否」を選択します。画面を閉じた場合も拒否として扱います。サーバー設定の「常に承認」をONにすると、そのサーバーの確認をすべて省略します。許可リストからツールを削除して保存すると、再び確認が必要になります。設定に未登録のサーバーは、今回だけの許可に対応します。

ワークフローの `mcp`・`command` ノードは、`confirm: "false"` で自動実行を含むそのノードのMCP承認を省略できます。ノード編集画面では「MCPツール実行の確認」で設定します。その後MCP Apps上で行う対話的な呼び出しには、サーバーの承認設定が適用されます。`command` の `vaultTools: "readOnly"` は組み込みVaultツールを検索・読み取りに制限する設定で、外部MCPの権限とは別です。
...
> **Thinking モード:** 対応するOpenAI / Gemini APIモデルでは、モデル選択欄の横で推論レベルを指定できます。`default`はプロバイダー既定、`none`は推論を無効化できるモデルで選択できます。完全に無効化できないGeminiモデルでは、対応する最低レベルを表示します。プロンプトのキーワードによる自動切り替えは行いません。

### OpenAI

| モデル | 説明 |
|-------|-------------|
| GPT-6 Astra | 複雑なエンドツーエンド作業向けの最高性能モデル |
