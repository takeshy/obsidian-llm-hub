...
**ダッシュボードを作成する:**
- コマンド: **「LLM Hub: ダッシュボードを作成」** — `Dashboards/` 配下に新しいボードを作成して開きます
- またはチャットで AI に依頼（組み込みの **dashboard** エージェントスキルが `.dashboard` ファイルと、その元になる `.base` ファイルを作成します）

**編集モード:** **編集**をクリックするとウィジェットの移動・リサイズ・追加・設定ができ、**完了**で表示モードに戻ります。グリッドはレスポンシブで、狭い画面ではウィジェットが 1 列に再配置されます。すべての編集は自動保存されます。

## ウィジェットの種類

編集モードで **+ ウィジェットを追加** をクリックして種類を選びます:
...
ノートをドラッグ＆ドロップのボードに変えます。カードは **タグ** および/または **フォルダ** フィルターに一致するノートで、フロントマターの **ステータスプロパティ** によって列にグループ化されます。カードを別の列にドラッグすると、そのノートのステータスが更新され、ノートのフロントマターに直接書き込まれます。ボードは **表示モード** で完全に操作でき、カードを動かすのに編集モードに入る必要はありません。

![カンバンボード](docs/images/dashboard_kanban.png)

ボード定義は `Dashboards/Kanbans/` 配下の再利用可能な `.kanban` YAML ファイルとして保存されます。既存のインライン定義は自動的にそこへ移行されます。複数のダッシュボードから同じファイルを参照でき、1つのウィジェット設定で行った変更は共有ファイルに反映されます。ヘッダーの一時タグ絞り込みに加え、表示項目は検出された frontmatter プロパティから選択でき、別名や項目名の非表示を指定できます。`file.path` / `file.name` / `file.content` / `file.mtime` / `file.ctime` は明示的に選択した場合だけ表示され、最大文字数は `file.content` に指定できます。
...
- **シークレットの編集** — 詳細モーダルから値・説明・公開メタデータを更新できます。ファイルは暗号化されたまま保存されます。
- **保存時の暗号化** — 平文値はロック解除中にメモリ上でのみ使用され、暗号化されていない状態で Vault に保存されることはありません。

![Secret Manager 編集](docs/images/secret_manager_edit.png)
...
`/v1/chat/completions` に対応していても `/v1/models` を提供しないプロキシやゲートウェイでは、**カスタム**を選択し、プロバイダー設定でモデルIDを手動入力してください。ベースURL末尾の `/v1` は、付けても付けなくても利用できます。
