...
| **Kanban** | 노트를 드래그 가능한 카드로 상태 열에 그룹화하여 표시 | `tag`/`folder` 필터, `statusProperty`, `columns` |

**Base** 및 **Workflow** 위젯에는 설정 패널을 벗어나지 않고 기반이 되는 `.base` 파일이나 워크플로를 만들 수 있는 **AI로 만들기** 버튼이 있습니다.
