...
- **대시보드** - Bases 뷰, 노트, 웹 페이지, 워크플로 출력을 반응형 위젯 그리드에 배치
...
> - **Antigravity CLI**: [Antigravity CLI](https://antigravity.google) 설치 후, `agy`로 인증
...
> [!NOTE]
> Vault 파일 @멘션은 파일 경로만 삽입하며 AI가 도구를 통해 내용을 읽습니다. CLI 모델이나 Ollama/OpenCode 같은 marker-only 로컬 LLM 프레임워크에서는 작동하지 않습니다. **LM Studio / vLLM / AnythingLLM** 같은 tools-capable 로컬 LLM은 로드된 모델이 Function Calling을 지원하면 vault 도구로 파일을 읽을 수 있습니다. Antigravity CLI는 셸을 통해 파일을 읽을 수 있지만, 응답 형식이 다를 수 있습니다.
...
| 조건 | 기본 모드 | 변경 가능 |
|------|-----------|-----------|
| CLI 모델 (Antigravity/Claude/Codex CLI) | Vault: 끄기 | 아니오 |
| 로컬 LLM | Vault: 끄기 | 아니오 |
...
**설정 → Workspace → LLM vault tool folders** 에서 LLM이 실행하는 vault 도구가 접근할 수 있는 vault 폴더를 제한할 수 있습니다. 이 설정은 API 프로바이더와 tools-capable 로컬 LLM, 그리고 LLM이 실행한 skill workflow에 적용됩니다. 비워 두면 전체 vault를 허용합니다. CLI 모델은 이러한 vault 도구를 사용하지 않으므로 이 설정으로 제한되지 않습니다.
...
- **CLI 모드 지원** - Antigravity CLI, Claude CLI, Codex CLI 백엔드에서도 스킬 사용 가능
...
- **Vault 도구** — AI가 플러그인 설정에 따라 노트를 읽고, 쓰고, 검색할 수 있으며 Workspace LLM vault 도구 폴더 제한도 적용됩니다
...
위젯의 반응형 그리드로 나만의 **홈/개요 페이지**를 구성합니다. 대시보드는 `.dashboard` 파일로, **Bases 뷰**, **노트**, **웹 페이지**, **워크플로 출력**, **칸반 보드**를 드래그·리사이즈 가능한 그리드에 배치합니다. 일반 노트처럼 열기만 하면 실시간으로 편집 가능한 보드가 표시됩니다.

![대시보드](docs/images/dashboard.png)

**대시보드 만들기:**
- 명령: **"LLM Hub: 대시보드 만들기"** — `Dashboards/` 아래에 새 보드를 만들고 엽니다
- 또는 채팅에서 AI에 요청（내장 **dashboard** 에이전트 스킬이 `.dashboard` 파일과 그 기반이 되는 `.base` 파일을 만들어 줍니다）

**편집 모드:** **편집**을 클릭하면 위젯을 이동·리사이즈·추가·구성할 수 있고, **완료**로 보기 모드로 돌아갑니다. 그리드는 반응형이며 좁은 화면에서는 위젯이 단일 열로 재배치됩니다. 모든 편집은 자동으로 저장됩니다.

## 위젯 유형

편집 모드에서 **+ 위젯 추가**를 클릭하여 유형을 선택합니다:

![위젯 추가](docs/images/dashboard_widgets.png)

| 위젯 | 표시 내용 | 주요 설정 |
|--------|-------|------------|
| **Base** | Obsidian 네이티브 Bases UI(테이블 / 카드 / 리스트)를 통한 `.base` 파일의 이름이 지정된 뷰 | `base` 경로, `view` 이름 |
| **Markdown** | 기존 노트를 인라인으로 렌더링 | 노트 `path` |
| **Web Embed** | iframe 안의 웹 페이지 | `url` |
| **Workflow** | 워크플로를 헤드리스로 실행한 출력을 Markdown 또는 HTML로 렌더링 | `workflow` 경로, `output`, `refreshInterval` |
| **Kanban** | 노트를 드래그 가능한 카드로 상태 열에 그룹화하여 표시 | `tag`/`folder` 필터, `statusProperty`, `columns`, `displayFields` |

**Base** 및 **Workflow** 위젯에는 설정 패널을 벗어나지 않고 기반이 되는 `.base` 파일이나 워크플로를 만들 수 있는 **AI로 만들기** 버튼이 있습니다. Base의 경우 AI가 작성 전에 읽기 전용 도구로 노트를 검사할 수 있으며, **AI로 편집**은 적용 전에 다듬을 수 있도록 diff와 추가 지시 입력란을 보여줍니다.

## 칸반 보드

노트를 드래그 앤 드롭 보드로 바꿉니다. 카드는 **태그** 및/또는 **폴더** 필터에 일치하는 노트이며, frontmatter의 **상태 속성**에 따라 열로 그룹화됩니다. 카드를 다른 열로 드래그하면 해당 노트의 상태가 업데이트되어 노트의 frontmatter에 직접 기록됩니다. 보드는 **보기 모드**에서 완전히 상호작용할 수 있으며, 카드를 옮기기 위해 편집 모드로 들어갈 필요가 없습니다.

![칸반 보드](docs/images/dashboard_kanban.png)

- **제목 및 새로 만들기** — 헤더에는 선택적 보드 제목(하나의 대시보드에 여러 보드를 둘 때 유용)과 **새로 만들기** 버튼이 표시됩니다. 새로 만들기 버튼은 제목 입력과 열 선택 대화 상자를 열고, 보드의 필터(폴더, 태그, 상태)에 이미 일치하는 노트를 만듭니다.
- **미리보기 및 열기** — 카드를 클릭하면 대화 상자에서 노트를 미리 볼 수 있으며, 대화 상자의 열기 아이콘으로 새 탭에서 노트를 엽니다.
- **열** — 색상으로 구분되며 완전히 구성할 수 있습니다. 선택적 "미지정" 열은 상태가 어떤 열과도 일치하지 않는 카드를 모읍니다.
- **표시 필드** — 각 카드의 제목 아래에 표시할 추가 frontmatter 속성(예: `priority`, `due`)을 나열합니다.

편집 모드의 위젯 설정에서 모든 것을 구성할 수 있습니다:

![칸반 설정](docs/images/dashboard_kanban_edit.png)
...
>
> 이러한 도구에는 **설정 → Workspace → LLM vault tool folders** 가 적용됩니다. API 프로바이더와 LLM이 실행한 skill workflow에 사용되는 것과 같은 폴더 제한입니다.
>
...
### CLI 모드 (Antigravity / Claude / Codex)

**Antigravity CLI:**
1. [Antigravity CLI](https://antigravity.google) 설치
2. `agy`로 인증
3. Antigravity CLI 섹션에서 "Verify" 클릭
...
3. **`.cmd` / `.bat` 래퍼** — 예: `C:\Users\YourName\AppData\Roaming\npm\agy.cmd`. `cmd.exe`를 거쳐야 하므로 프롬프트의 `&`, `|`, `>`, `^`, `%VAR%` 등이 오작동할 수 있습니다.

PowerShell에서 `Get-Command agy` / `Get-Command claude` / `Get-Command codex`를 실행하여 래퍼 경로를 확인한 뒤, 이를 직접 입력(옵션 3)하거나 인접한 `.js` / `.exe`로 이동해 더 안전한 옵션을 선택하세요.
</details>
...
1. 터미널을 열고 실행:
   ...
   which agy
   ```
2. 표시된 경로 (예: `/home/user/.local/bin/agy`)를 CLI 경로 설정에 입력
...
큰 Vault에서는 폴더별로 여러 RAG 설정을 만들고 각각 동기화한 다음, 별도의 RAG 설정에서 **Combine internal**을 선택할 수 있습니다. 동기화된 원본 설정을 선택하면 하나의 Chat/Search 선택기에서 함께 검색할 수 있습니다.

동기화 중에는 변경된 파일을 작은 파일 배치로 처리하고 저장합니다. 이는 RAG chunk size 설정과는 별개의 동기화 저장 단위입니다. PDF 텍스트 추출에 실패하면 동기화 후 실패한 PDF 경로가 표시되고 checksum이 저장되며, 인덱싱된 파일 목록에는 `0 chunks`로 표시됩니다. PDF 파일이 변경되지 않는 한 이후 동기화에서 다시 시도하지 않습니다. 다시 가져오려면 PDF 파일 이름을 변경하거나, 파일을 수정하거나, RAG 인덱스를 다시 빌드하세요.

**외부 인덱스:**

vault에서 동기화하는 대신 사전 구축된 인덱스를 사용:

1. **Index mode**를 **External**로 설정
2. `index.json` 및 `vectors.bin`을 포함하는 절대 인덱스 디렉터리를 한 줄에 하나씩 입력
...
**CLI 제공자 (선택 사항):**
- CLI 모드가 활성화되면 외부 CLI 도구 (agy, claude, codex)가 child_process를 통해 실행됩니다
