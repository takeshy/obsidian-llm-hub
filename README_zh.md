...
- **仪表板** - 在响应式小组件网格中排列 Bases 视图、笔记、网页和工作流输出
...
用响应式小组件网格构建你专属的**主页/概览页**。仪表板是一个 `.dashboard` 文件，它将 **Bases 视图**、**笔记**、**网页**、**工作流输出**和**看板**排列在一个可拖拽和调整大小的网格中——像打开任何笔记一样打开它，即可看到一个可实时编辑的面板。

![仪表板](docs/images/dashboard.png)

**创建仪表板：**
- 命令：**“LLM Hub: 创建仪表板”** — 在 `Dashboards/` 下创建一个新面板并打开它
- 或在聊天中请求 AI（内置的 **dashboard** 智能体技能会为你创建 `.dashboard` 文件以及作为其基础的 `.base` 文件）

**编辑模式：** 点击**编辑**即可移动、调整大小、添加和配置小组件；点击**完成**进入查看模式。网格是响应式的——在窄屏上小组件会重新排列为单列。所有编辑都会自动保存。

## 小组件类型

在编辑模式下点击 **+ 添加小组件** 以选择类型：

![添加小组件](docs/images/dashboard_widgets.png)

| 小组件 | 显示 | 主要配置 |
|--------|-------|------------|
| **Base** | 通过 Obsidian 原生 Bases UI（表格 / 卡片 / 列表）显示 `.base` 文件的命名视图 | `base` 路径、`view` 名称 |
...
将笔记变成拖放式看板。卡片是匹配 **标签** 和/或 **文件夹** 筛选的笔记，按 frontmatter 中的 **状态属性** 分组到各列。将卡片拖到另一列即可更新该笔记的状态——直接写回笔记的 frontmatter。看板在 **查看模式** 下完全可交互；无需进入编辑模式即可移动卡片。
