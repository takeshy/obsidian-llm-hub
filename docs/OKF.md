...
LLM Hub can use Open Knowledge Format (OKF) bundles as chat knowledge sources.
...
When OKF is enabled, LLM Hub reads Markdown files from the configured directory recursively and injects a compact summary into the chat system prompt. This gives the model curated domain context without requiring a separate server.
...
2. Enable OKF in LLM Hub settings.
