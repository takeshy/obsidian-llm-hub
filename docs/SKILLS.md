...
External skills must include both `SKILL.md` and `manifest.json`. The manifest records the skill ID, version, description, and plugin compatibility, so LLM Hub can skip incompatible or outdated packages safely.
