...
스킬은 CLI 백엔드(Antigravity CLI, Claude CLI, Codex CLI)에서도 작동합니다. CLI 프로바이더는 Function Calling을 지원하지 않으므로, 스킬 워크플로는 텍스트 기반 규칙을 사용합니다: AI가 `[RUN_WORKFLOW: workflowId]` 마커를 출력하면, 플러그인이 자동으로 워크플로를 실행하고 결과를 표시합니다.
