...
export const ko: Record<string, string> = {
  ...
  "settings.cliPathModal.windowsNote": "자동 감지를 위해 비워두세요 (npm 전역 설치 또는 PATH의 래퍼 감지). 재정의하려면 다음 중 하나를 지정하세요:\n• `...\\npm\\node_modules\\@google\\gemini-cli\\dist\\index.js` — node를 통해 실행 (가장 안전)\n• `...\\claude.exe` — 독립 실행형 실행 파일\n• `...\\npm\\gemini.cmd` — cmd.exe를 통해 실행 (프롬프트의 메타문자로 오작동 가능)",
