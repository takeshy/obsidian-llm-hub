...
export const zh: Record<string, string> = {
  ...
  "settings.cliPathModal.windowsNote": "留空以自动检测（检测 npm 全局安装或 PATH 中的包装脚本）。如需覆盖，请指向以下任一项：\n• `...\\npm\\node_modules\\@google\\gemini-cli\\dist\\index.js` — 通过 node 运行（最安全）\n• `...\\claude.exe` — 独立可执行文件\n• `...\\npm\\gemini.cmd` — 通过 cmd.exe 运行（提示词中的特殊字符可能导致异常）",
